...
async function main() {
	...
	const provider = new E2BSandboxProvider({ apiKey: process.env.E2B_API_KEY });
	...
	try {
		...
		assert(manager.list().some((s) => s.id === id), "sandbox session is in the manager's registry");
		...
		assert(screen.includes("sandbox-e2b-marker-789"), `screen shows the marker\n--- screen ---\n${screen}`);
		...
		if (id) {
			try { await manager.close(id); } catch { /* best-effort */ }
		}
	}
	process.exit(ok ? 0 : 1);
}
