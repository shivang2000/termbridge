...
interface CmdResult { exitCode: number; stdout: string; stderr: string; error?: string; }
...
function makeMockFactory(results: CmdResult[] = [{ exitCode: 0, stdout: "", stderr: "" }]) {
	const createCalls: Array<{ template?: string; envs?: Record<string, string>; apiKey?: string; timeoutMs?: number; metadata?: Record<string, string> }> = [];
		...
		commands: {
			run: async (cmd: string) => {
				...
				if (r.exitCode !== 0) {
					...
					err.exitCode = r.exitCode; err.stdout = r.stdout; err.stderr = r.stderr; err.error = "non-zero exit";
					throw err;
				}
				return r;
			},
		},
		kill: async () => { killCalls++; return true; },
	};
	const sandboxFactory = async (opts: { template?: string; envs?: Record<string, string>; apiKey?: string; timeoutMs?: number; metadata?: Record<string, string> }) => {
		createCalls.push(opts); return sandbox;
	};
	return { sandboxFactory, createCalls, runCalls, get killCalls() { return killCalls; } };
}

describe("E2BSandboxProvider", () => {
	it("has name 'e2b'", () => {
		const p = new E2BSandboxProvider({ apiKey: "k", sandboxFactory: makeMockFactory().sandboxFactory });
	...
	it("ensure provisions a sandbox via the factory with template/envs/apiKey/metadata", async () => {
		...
		const p = new E2BSandboxProvider({ apiKey: "k", template: "base", timeoutMs: 1000, sandboxFactory: m.sandboxFactory });
	...
	it("exec shell-joins the argv and returns the result", async () => {
		...
		await p.ensure({ name: "s", cwd: "/w" });
		const res = await p.exec(["tmux", "-L", "termbridge", "list-sessions", "-F", "#{session_name}"]);
