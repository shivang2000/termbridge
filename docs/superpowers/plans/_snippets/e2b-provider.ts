...
interface E2BSandboxLike {
	...
	commands: {
		run(cmd: string, opts?: { timeoutMs?: number }): Promise<{ exitCode: number; stdout: string; stderr: string; error?: string }>;
	};
...
export class E2BSandboxProvider implements SandboxProvider {
	...
	async ensure(opts: {
		...
		await this.execRaw("command -v tmux >/dev/null 2>&1 || (apt-get update -y && apt-get install -y tmux)");
	}
